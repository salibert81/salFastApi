...
- 🎯 Entry points to execute your program using `python -m <salfastapi>` or `$ salfastapi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/salibert81/salFastApi/branch/main/graph/badge.svg?token=salFastApi_token_here)](https://codecov.io/gh/salibert81/salFastApi)
[![CI](https://github.com/salibert81/salFastApi/actions/workflows/main.yml/badge.svg)](https://github.com/salibert81/salFastApi/actions/workflows/main.yml)

Awesome salfastapi created by salibert81

## Install

from source
```bash
git clone https://github.com/salibert81/salFastApi salfastapi
cd salfastapi
make install
```
...
```bash
pip install salfastapi
```

## Executing

```bash
$ salfastapi run --port 8080
```

or

```bash
python -m salfastapi run --port 8080
```

or

```bash
$ uvicorn salfastapi:app
```
...
```bash
❯ salfastapi --help
Usage: salfastapi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ salfastapi create-user --help
Usage: salfastapi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
salfastapi create-user admin admin --superuser
```
...
Run with `salfastapi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `salfastapi create-user`.
...
-----------------------------------------------------
salfastapi/__init__.py              4      0   100%
salfastapi/app.py                  16      1    94%
salfastapi/cli.py                  21      0   100%
salfastapi/config.py                5      0   100%
salfastapi/db.py                   10      0   100%
salfastapi/models/__init__.py       0      0   100%
salfastapi/models/content.py       47      1    98%
salfastapi/routes/__init__.py      11      0   100%
salfastapi/routes/content.py       52     25    52%
salfastapi/routes/security.py      15      1    93%
salfastapi/routes/user.py          52     26    50%
salfastapi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in salfastapi/default.toml.
